...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
  	<th width="5%">No.</th>
    <th width="7%">Kode</th>
    <th width="83%">Nama Alternatif</th>
	<script type="text/javascript">
    function konTambah() {
            window.location = "?page=alternatif-input&act=tambah";
    }
    </script>
...
$perhalaman=15;
$query_part ="SELECT id_alternatif, kode, alternatif FROM alternatif
	    	  WHERE kode LIKE '%$txtcari%' OR alternatif LIKE '%$txtcari%'";
...
$nomor=$halamannya;
$query="SELECT id_alternatif, kode, alternatif FROM alternatif
	    WHERE kode LIKE '%$txtcari%' OR alternatif LIKE '%$txtcari%'
		ORDER BY kode ASC LIMIT $halamannya, $perhalaman" ;
...
function konfirmasi<?php echo $dataquery[0]; ?>() {
	...
	if (answer){
		window.location = "aksi_alternatif.php?act=hapus&id=<?php echo"$dataquery[0]"; ?>";
	}
}
</script>
  <tr>
    ...
    <td align="center">
    <a href="?page=alternatif-input&act=edit&id=<?php echo"$dataquery[0]"; ?>">
...
<div style="margin-top:15px; padding-top:10px; clear:both; line-height:normal;">
		<div style="padding:3px; margin:1px; border:1px solid; border-color:#999999; background-color:#666666; color:#CCCCCC; float:left;">Halaman :</div> 
          <?php
			...
			{
			?>
			<div style="padding:3px; margin:1px; border:1px solid; border-color:#77BBFF; <?php if (($halaman+1)==$j) { echo"background-color:#FFF"; } else {  echo"background-color:#B0D8FF"; } ?>; float:left;"><a href="?page=alternatif&halaman=<?php echo"$j"; ?><?php if(@$kat<>"") { echo"&kat=@$kat"; } ?>" title="Halaman : <?php echo"$j"; ?>" style="text-decoration:none; color:#06C;"><?php echo"$j"; ?></a></div>
			<?php }
		?>
</div>